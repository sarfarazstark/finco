...
async function main() {
	...
	const themeByHex = Object.fromEntries(themes.map((t) => [t.hex, t]));
	...
	await prisma.balance.upsert({
		...
		update: { current: 4836.0, income: 3814.25, expenses: 1700.5 },
		create: {
			userId: user.id,
			current: 4836.0,
			income: 3814.25,
	...
	// Delete existing transactions for this user to avoid duplicates on re-seed
	await prisma.transaction.deleteMany({ where: { userId: user.id } });

	await prisma.transaction.createMany({
		data: transactionsData.map((t) => ({
			userId: user.id,
			categoryId: categoryByName[t.category].id,
			name: t.name,
			...
			recurringInterval: t.recurring ? RecurringInterval.MONTHLY : null,
		})),
	});
	...
	await prisma.budget.createMany({
		data: budgetsData.map((b) => ({
			userId: user.id,
			categoryId: categoryByName[b.category].id,
			maximum: b.maximum,
			themeId: themeByHex[b.theme].id,
		})),
	});
	...
	await prisma.pot.createMany({
		data: potsData.map((p) => ({
			...
			target: p.target,
			total: p.total,
			themeId: themeByHex[p.theme].id,
		})),
	});

	console.log(`🏺 Created ${potsData.length} pots`);
